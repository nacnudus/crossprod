...
The [nzcrash](https://github.com/nacnudus/nzcrash) package redistributes crash
statistics already available from the New Zealand Transport Agency, but in a
more convenient form.  ***Edit: The data is no longer available from the
NZTA.***
...
`vehicles` dataset by the combination of the `id` and `vehicle_id` columns. This
is most useful when the resulting table is also joined to the `crashes` dataset.

## Up-to-date-ness

The data was last scraped from the NZTA website on 2015-07-20. At that time, the
NZTA had published data up to the 2015-03-10.

```{r}
glimpse(crashes)
glimpse(causes)
glimpse(vehicles)
glimpse(objects_struck)
```

## Accuracy

The [NZTA](http://www.transport.govt.nz/research/roadtoll/#5), doesn't agree
with
[itself](http://www.transport.govt.nz/research/roadtoll/annualroadtollhistoricalinformation/)
about recent annual road tolls, and this dataset gives a third opinion.
...
99.9% of crashes have coordinates. These have been converted from the NZTM
projection to the WGS84 projection for convenience with packages like `ggmap`.
...
There can be many objects struck in one crash, so these are recorded in a
separate `objects_struck` dataset that can be joined to `crashes` by the common
`id` column.
...
That's odd -- where are speed, alcohol, and restraints? They're given in
`cause_subcategory`.
...
There's nothing there about speed limit violations, because it's impossible to
tell what speed a vehicle was going at when it crashed.
...
```{r, fig.show = "hold"}
levels(causes$cause) <- str_wrap(levels(causes$cause), 13) # Wrap facet labels
crashes %>%
  inner_join(causes, by = "id") %>%
...
This time, join `causes` to both `vehicles` and `crashes` to assess the drunken
cyclist menace.
...
Check out the [package](https://github.com/nacnudus/nzcrash) on Github at
